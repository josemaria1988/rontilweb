...
function agregar_carrito(id) {
    ...
    let index = array_carrito.findIndex(item => item.id == id);
    let existe = array_carrito.some(prod => prod.id === id);
    existe == true ? array_carrito[index].cantidad++ : array_carrito.push(agregar_producto);
...
function actualizar_carrito(agregar_producto) {
    ...
    array_carrito.forEach(agregar_producto => {
        ...
        btn_eliminar.addEventListener('click', () => {
            ...
            localStorage.setItem('carrito', JSON.stringify(array_carrito));
            actualizar_carrito();
            descuento_de_usuario(precio_total);
            Swal.fire({
                icon: 'error',
                title: 'Listo!',
...
let solicitar_descuento = document.getElementById("solicitar-descuento");
solicitar_descuento.addEventListener('submit', (e) => {
    e.preventDefault(e);
    descuento_de_usuario(e);
    actualizar_carrito();
});

function descuento_de_usuario(precio_total) {
    let usuario = localStorage.getItem('usuario', JSON.parse(usuario));
    let codigo_descuento = document.getElementById("codigo-descuento");

    if (codigo_descuento.value == 494595) {
...
fetch("https://api.openweathermap.org/data/2.5/weather?q=Montevideo&lang=es&units=metric&appid=bd20fe08f0714413cda1f9f63fe1ac7f")
